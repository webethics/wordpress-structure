...
$sum1 = 0;
		...
		<? if ( $order->has_status( 'failed' ) ) : ?>
			...
			<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions">
				<a href="<? echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button pay"><? esc_html_e( 'Pay', 'woocommerce' ); ?></a>
				<? if ( is_user_logged_in() ) : ?>
					<a href="<? echo esc_url( wc_get_page_permalink( 'myaccount' ) ); ?>" class="button pay"><? esc_html_e( 'My account', 'woocommerce' ); ?></a>
				<? endif; ?>
			</p>
		...
		$item_totals = $order->get_order_item_totals();
			...
			?>
			<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><? echo apply_filters( 'woocommerce_thankyou_order_received_text', esc_html__( 'Thank you. Your order has been received.', 'woocommerce' ), $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>

			<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details">

				<li class="woocommerce-order-overview__order order">
					<? esc_html_e( 'Order number:', 'woocommerce' ); ?>
					<strong><? echo $order->get_order_number(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
				</li>

				<li class="woocommerce-order-overview__date date">
					<? esc_html_e( 'Date:', 'woocommerce' ); ?>
					<strong><? echo wc_format_datetime( $order->get_date_created() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
				</li>

				<? if ( is_user_logged_in() && $order->get_user_id() === get_current_user_id() && $order->get_billing_email() ) : ?>
					<li class="woocommerce-order-overview__email email">
						<? esc_html_e( 'Email:', 'woocommerce' ); ?>
						<strong><? echo $order->get_billing_email(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
					</li>
				<? endif; ?>

				<li class="woocommerce-order-overview__total total">
					<? esc_html_e( 'Total:', 'woocommerce' ); ?>
					<strong><? echo $order->get_formatted_order_total(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
				</li>

				<? if ( $order->get_payment_method_title() ) : ?>
					<li class="woocommerce-order-overview__payment-method method">
						<? esc_html_e( 'Payment method:', 'woocommerce' ); ?>
						<strong><? echo wp_kses_post( $order->get_payment_method_title() ); ?></strong>
					</li>
				<? endif; ?>
	...
	<? else : ?>
		...
		<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><? echo apply_filters( 'woocommerce_thankyou_order_received_text', esc_html__( 'Thank you. Your order has been received.', 'woocommerce' ), null ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
